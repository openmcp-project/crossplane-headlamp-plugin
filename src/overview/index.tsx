      ...
      {loading ? (
        ...
        <Box display="flex" alignItems="center" gap={3}>
          <Box position="relative" flexShrink={0} style={{ width: size, height: size }}>
            ...
            >
              <Typography style={{ fontWeight: 700, fontSize: size * 0.22, lineHeight: 1 }}>
                {total}
              </Typography>
              <Typography variant="caption" color="textSecondary" style={{ fontSize: size * 0.11 }}>
                total
              </Typography>
            </Box>
          </Box>
...
function ProviderCard({
  ...
  const allOk =
    ...
    healthyCond?.status === 'True' &&
    installedCond?.status === 'True';
  const anyError =
  ...
  const mrReady = providerMRs.filter(
    (i: any) => i.status?.conditions?.find((c: any) => c.type === 'Ready')?.status === 'True'
  ).length;
  const mrSynced = providerMRs.filter(
    (i: any) => i.status?.conditions?.find((c: any) => c.type === 'Synced')?.status === 'True'
  ).length;
    ...
    <Paper elevation={2} style={{ borderLeft: `4px solid ${borderColor}`, overflow: 'hidden' }}>
      ...
      <Box px={2} py={1.5} display="flex" alignItems="center" gap={3} flexWrap="wrap">
        ...
        <Box>
          ...
          >
            ProviderConfig{configCount !== 1 ? 's' : ''}
            {configCount !== null ? ` (${configCount})` : ''}
                      ...
                      <>
                        <span
                          style={{
                            ...
                            fontWeight: 600,
                            color: '#fff',
                            background: readyOk ? '#4caf50' : readyBad ? '#f44336' : '#9e9e9e',
                          }}
                        >
                          Ready
                        </span>
                        <span
                          style={{
                            ...
                            fontWeight: 600,
                            color: '#fff',
                            background: syncedOk ? '#1976d2' : syncedBad ? '#f44336' : '#9e9e9e',
                          }}
                        >
                          Synced
                        </span>
...
export default function CrossplaneOverview() {
  ...
  if (error) {
    ...
    const notInstalled =
      errMsg.includes('404') || errMsg.includes('not found') || errMsg.includes('no kind');
    return (
      <Box p={3}>
  ...
  const mrReady = mrItems.filter(
    (i: any) => i.status?.conditions?.find((c: any) => c.type === 'Ready')?.status === 'True'
  ).length;
  const mrSynced = mrItems.filter(
    (i: any) => i.status?.conditions?.find((c: any) => c.type === 'Synced')?.status === 'True'
  ).length;
  const mrNotReady = mrTotal - mrReady;
  const mrNotSynced = mrTotal - mrSynced;
  ...
  const mrSyncSlices: DonutSlice[] = [
    ...
    },
    {
      ...
      label: 'Not Synced',
      onClick:
        mrNotSynced > 0 ? () => history.push(`${resourcesBase}?status=not-synced`) : undefined,
    },
  ];
